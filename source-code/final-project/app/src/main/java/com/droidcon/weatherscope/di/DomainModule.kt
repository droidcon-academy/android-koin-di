...
object DomainModule {
    @Factory
    fun provideCurrentWeatherUseCase(repo: WeatherRepository): GetCurrentWeatherUseCase =
        GetCurrentWeatherUseCaseImpl(repo)

    @Single
    fun provideWeatherDomain(useCase: GetCurrentWeatherUseCase): WeatherDomain =
        WeatherDomain(useCase)

    @Factory
    fun provideForecastUseCase(repo: WeatherRepository): GetWeatherForecastUseCase =
