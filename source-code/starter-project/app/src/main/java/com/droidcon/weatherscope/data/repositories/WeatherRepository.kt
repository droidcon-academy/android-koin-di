...
class WeatherRepository(
    ...
    private fun getWeatherForecastByCoordinates(lat: Double, lon: Double): Flow<ApiCallResult<WeatherForecastResponse>> =
        safeApiCall {
            apiService.getForecastByCoordinates(lat, lon)
        }

    fun getWeatherForecast(latitude: Double, longitude: Double): Flow<DataState<List<WeatherForecastItem>>> =
        flow {
            val localForecasts = localSource.getForecasts().firstOrNull() ?: emptyList()
            ...
            if (localForecasts.isNotEmpty() && isFresh) {
                emit(DataState.Success(localForecasts.map { it.toDomain() }))
                ...
                try {
                    ...
                    val forecastEntities = domainForecasts.map { it.toEntity() }
                    localSource.saveForecasts(forecastEntities)
                    emit(DataState.Success(domainForecasts))
                } catch (e: Exception) {
                    emit(DataState.Error("While loading forecasts from api, ${e.message ?: "Unknown error"}"))
                }
            }
        }.catch { exception ->
